# 📋 AI × Web3 School 个人学习计划

> 更新日期：2026-05-19
> 策略：两边有基础 → **Bridge-first，按需补盲，4 周对齐官方进度**

---

## 🎯 总目标

跟随 AI × Web3 School 第一期共学营节奏，完成基础共学 → 方向选择 → 实战练习 → Hackathon 项目提交的完整路径，并通过 WCB Showcase 留下可继续推进的项目成果。

---

## ⏱️ 4 周路线图（对齐官方大纲）

---

### Week 1｜共学营：AI 与 Web3 基础知识

**核心目标**：建立共同语言，理解 LLM、agent、workflow、tool use、钱包、签名、合约在同一任务链里的关系

**AI 基础（按需补盲）**
- [x] [LLM](https://aiweb3.school/zh/handbook/ai/llm/) — 模型能力边界 ✅ 05-20
- [x] [Context](https://aiweb3.school/zh/handbook/ai/context/) — 上下文管理 ✅ 05-21
- [ ] [Agent](https://aiweb3.school/zh/handbook/ai/agent/) — 工具调用 & 多步执行
- [ ] [MCP](https://aiweb3.school/zh/handbook/ai/mcp/) — 模型上下文协议
- [ ] [AI Coding](https://aiweb3.school/zh/handbook/ai/ai-coding/) — Vibe Coding / AI 辅助开发

**Web3 基础（按需补盲）**
- [ ] [Wallet](https://aiweb3.school/zh/handbook/web3/wallet/) — 身份 & 签名入口
- [ ] [Transaction & Gas](https://aiweb3.school/zh/handbook/web3/transaction/) — 交易与 Gas
- [ ] [Smart Contract](https://aiweb3.school/zh/handbook/web3/smart-contract/) — 合约基础
- [ ] [Testnet](https://aiweb3.school/zh/handbook/web3/testnet/) — 测试网操作

**本周实践交付**
- [ ] 完成 Vibe Coding / Hermes Agent 实践，记录过程
- [ ] 创建测试钱包，完成测试网转账
- [ ] 完成合约部署或调用（测试网）
- [ ] 打卡记录：成功案例 + 失败案例 + 人工修正经历

---

### Week 2｜共学营：AI × Web3 交叉方向

**核心目标**：进入真实问题空间，理解 Bridge 核心概念，选定方向并形成 Proposal

**Bridge 核心概念**
- [ ] [Chain-aware Context](https://aiweb3.school/zh/handbook/bridge/chain-aware-context/)
- [ ] [Web3 Tool Use](https://aiweb3.school/zh/handbook/bridge/web3-tool-use/)
- [ ] [Agent Workflow](https://aiweb3.school/zh/handbook/bridge/agent-workflow/)
- [ ] [Agent Wallet](https://aiweb3.school/zh/handbook/bridge/agent-wallet/)
- [ ] [Machine Payment](https://aiweb3.school/zh/handbook/bridge/machine-payment/)

**方向选择（五选一）**
- [ ] 阅读 [Agentic Commerce / Payment](https://aiweb3.school/zh/handbook/tracks/agentic-commerce/)
- [ ] 阅读 [Dev Tooling](https://aiweb3.school/zh/handbook/tracks/dev-tooling/)
- [ ] 阅读 [AI Security / Privacy](https://aiweb3.school/zh/handbook/bridge/ai-security/)
- [ ] 阅读 [AI × Governance / Coordination](https://aiweb3.school/zh/handbook/tracks/governance/)
- [ ] 或 Open Track

**本周交付**
- [ ] 选定一个方向
- [ ] 拆解「谁发起任务 / 谁执行 / 谁付款 / 谁验证 / 谁承担风险」
- [ ] 形成项目 Proposal（记录在 `hackathon/proposal.md`）
- [ ] 打卡：`notes/bridge/week2-summary.md`

---

### Week 3｜实践深化与 Hackathon 启动

**核心目标**：补足基础、完成小型实战练习，确定 Hackathon 项目计划

**补充深化（按方向选读）**
- [ ] [Agent Identity](https://aiweb3.school/zh/handbook/bridge/agent-identity/)
- [ ] [Agent Trust & Reputation](https://aiweb3.school/zh/handbook/bridge/agent-trust-and-reputation/)
- [ ] [Settlement & Escrow](https://aiweb3.school/zh/handbook/bridge/settlement-and-escrow/)
- [ ] [Verifiable AI](https://aiweb3.school/zh/handbook/bridge/verifiable-ai/)

**小型实战练习（选做）**
- [ ] Agent workflow + 钱包确认流程
- [ ] 链上收据 / 测试网支付记录
- [ ] 治理提案总结 Agent
- [ ] 风险控制策略 / 可验证执行记录

**Hackathon 启动确认**
- [ ] 确定赛道
- [ ] 确定项目题目 & 目标用户
- [ ] 规划技术路径
- [ ] 确定团队分工
- [ ] 制定 Demo 计划

**本周交付**：`hackathon/hackathon-plan.md`（含赛道、题目、目标用户、技术路径、分工、demo 计划）

---

### Week 4｜Hackathon 集中开发与 Demo 展示

**核心目标**：完成核心功能，准备最终提交材料

**开发清单**
- [ ] 完成核心功能实现
- [ ] 基础测试通过
- [ ] 编写 README
- [ ] 准备演示流程
- [ ] 记录必要的链上 / 工具调用记录
- [ ] 参加 Mentor Office Hour，获取反馈

**最终提交材料**
- [ ] 项目名称 & 赛道
- [ ] 目标用户 & 问题定义
- [ ] Demo link
- [ ] 代码仓库 & README
- [ ] 演示视频
- [ ] 链上验证材料（合约地址 / 测试网地址 / 交易哈希）

**本周交付**：`hackathon/final-submission.md`（整理全部提交材料）

---

## 📌 个人盲点记录

> 在学习过程中发现的真正不熟悉的点，随时更新

- [ ] <!-- 例：MCP 协议具体实现细节 -->
- [ ] <!-- 例：Account Abstraction 和 Session Key 的关系 -->

---

## 🔗 重要链接

- 官方 Handbook：https://aiweb3.school/zh/handbook/
- WCB 项目展示：https://app.buidlbox.io/
- 启动 Prompt：https://aiweb3.school/learning-agent.zh.txt
